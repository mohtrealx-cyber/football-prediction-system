...
class TicketBuilderTests(unittest.TestCase):
    ...
    def test_four_tickets_can_be_built(self):
        ...
        self.assertEqual([t.name for t in tickets], ["SAFE", "BALANCED", "AGGRESSIVE", "VALUE"])
    ...
    def test_every_ticket_has_at_least_three_matches(self):
        builder = TicketBuilder()
        tickets = builder.build(self.selections)
        self.assertTrue(all(len(t.selections) >= 3 for t in tickets))
    ...
    def test_duplicate_match_is_not_allowed_inside_ticket(self):
        duplicate = [
            Selection("M1", "A vs B", "1X", 1.35, 88, 8.2),
        ...
        for ticket in tickets:
            ids = [s.match_id for s in ticket.selections]
